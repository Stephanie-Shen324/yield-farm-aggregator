...
def get_price_history(search):
  '''
  Gets the historical price, market cap and volume data for a specified asset via Defi's geckoHistorical function, which grabs it straight from coingecko's API

  INPUTS:
  search: coingecko's historical API requires the NAME of an asset, however this function has been written such that the name or symbol of an asset can be entered

  OUTPUT:
  if the term entered is valid, it will return the historical dataframe for the asset, otherwise it will return an error message for invalid entry
  '''
  ...
  # If nothing is found, try searching for that term in the dictionary and search coingecko for its corresponding name (works if search term is the symbol of an asset)
...
def historical_iloss(base, quote, d):
  '''
  Combines the above functions with Defi Tools' iloss function to give the actual impermanent loss percentage over the last d days
...
def IL_calc(assets):
  ...
  if len(assets) < 2:
    ...
    # KeyError indicates missing asset data, append 'No data'
      ...
      ILoss = 'No data'
    # Most other errors indicate an unresponsive API on account of too many calls, try again 3 times with a 5 second wait
    except:
